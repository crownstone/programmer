"""
Copied from https://raw.githubusercontent.com/buckley-w-david/python-crypto/master/python_crypto/symmetric/rc5.py
MIT Licensed
...
def _expand_key(key: bytes, wordsize: int, rounds: int) -> typing.List[int]:
    ...
    def _align_key(key: bytes, align_val: int) -> typing.List[int]:
        while len(key) % align_val:
            key += (
                b"\x00"
            )  # Add 0 bytes until the key length is aligned to the block size
