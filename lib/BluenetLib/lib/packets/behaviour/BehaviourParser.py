...
from BluenetLib.lib.packets.behaviour.Behaviour import Behaviour
from BluenetLib.lib.packets.behaviour.BehaviourSubClasses import ActiveDays, BehaviourTimeContainer, BehaviourTime, \
    BehaviourPresence
from BluenetLib.lib.packets.behaviour.BehaviourTypes import DAY_START_TIME_SECONDS_SINCE_MIDNIGHT, BehaviourType, \
    BehaviourTimeType, BehaviourPresenceType
...
def BehaviourDictionaryParser(dict, dayStartTimeSecondsSinceMidnight = DAY_START_TIME_SECONDS_SINCE_MIDNIGHT):
    ...
    if profileIndex  is None: raise BluenetError.PROFILE_INDEX_MISSING
    if typeString    is None: raise BluenetError.TYPE_MISSING
    if data          is None: raise BluenetError.DATA_MISSING
    if activeDays    is None: raise BluenetError.ACTIVE_DAYS_MISSING
    ...
    if actionDict is None: raise BluenetError.BEHAVIOUR_ACTION_MISSING
    if timeDict   is None: raise BluenetError.BEHAVIOUR_TIME_MISSING

  
    intensity = getFromDict(actionDict, "data")
    
    if intensity is None: raise BluenetError.BEHAVIOUR_INTENSITY_MISSING 
    ...
    if presence is not None:
        if behaviourType == BehaviourType.twilight :
            raise BluenetError.TWILIGHT_CANT_HAVE_PRESENCE
    ...
    if endCondition is not None:
        if behaviourType == BehaviourType.twilight :
            raise BluenetError.TWILIGHT_CANT_HAVE_END_CONDITION
...
def ActiveDayParser(dict):
    ...
    if Monday is not None and Tuesday is not None and Wednesday is not None and Thursday is not None and Friday is not None and Saturday is not None and Sunday is not None:
        ...
        if activeDays.getMask() == 0:
            raise BluenetError.NO_ACTIVE_DAYS
        
        return activeDays
    else :
        raise BluenetError.ACTIVE_DAYS_INVALID
...
def TimeParser(dict, dayStartTimeSecondsSinceMidnight):
    ...
    if type is None: raise BluenetError.NO_TIME_TYPE
    ...
    if type == "ALL_DAY":
        ...
        if fromTime is None: raise BluenetError.MISSING_FROM_TIME
        if toTime   is None: raise BluenetError.MISSING_TO_TIME

        fromTimeType = getFromDict(fromTime, "type")
        toTimeType   = getFromDict(toTime,   "type")

        if fromTimeType is None: raise BluenetError.MISSING_FROM_TIME_TYPE
        if toTimeType   is None: raise BluenetError.MISSING_TO_TIME_TYPE
        ...
        if fromTimeType == "CLOCK":
            ...
            if fromData is None: raise BluenetError.MISSING_FROM_TIME_DATA

            hours   = getFromDict(fromData, "hours")
            minutes = getFromDict(fromData, "minutes")

            if hours   is None: raise BluenetError.MISSING_FROM_TIME_DATA
            if minutes is None: raise BluenetError.MISSING_FROM_TIME_DATA
            ...
            raise BluenetError.INVALID_TIME_FROM_TYPE
        ...
        if toTimeType == "CLOCK":
            ...
            if toData is None: raise BluenetError.MISSING_TO_TIME_DATA

            hours = getFromDict(toData, "hours")
            minutes = getFromDict(toData, "minutes")

            if hours is None: raise BluenetError.INVALID_TO_DATA
            if minutes is None: raise BluenetError.INVALID_TO_DATA
            ...
            raise BluenetError.INVALID_TIME_TO_TYPE
        
        return BehaviourTimeContainer(fromResult, toResult)
    else:
        raise BluenetError.INVALID_TIME_TYPE
...
def PresenceParser(dict):
    type = getFromDict(dict, "type")
    if type is None: raise BluenetError.NO_PRESENCE_TYPE

    if type == "IGNORE":
        return BehaviourPresence()
    elif type == "SOMEBODY" or type == "NOBODY":
        data = getFromDict(dict, "data")
        if data is None: raise BluenetError.NO_PRESENCE_DATA

        delay = getFromDict(dict, "delay")
        if delay is None: raise BluenetError.NO_PRESENCE_DELAY

        dataType = getFromDict(data, "type")
        if dataType is None: raise BluenetError.NO_PRESENCE_TYPE
        ...
        if dataType == "SPHERE":
            ...
            if locationIdArray is None: raise BluenetError.NO_PRESENCE_LOCATION_IDS
            ...
            raise BluenetError.NO_PRESENCE_DATA
    else :
        raise BluenetError.INVALID_PRESENCE_TYPE
...
def EndConditionParser(dict):
    ...
    if type     is None: raise BluenetError.NO_END_CONDITION_TYPE
    if presence is None: raise BluenetError.NO_END_CONDITION_PRESENCE
